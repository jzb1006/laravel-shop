...
class PaymentController extends Controller
{
    ...
    public function payByAlipay(Order $order,Request $request){
        ...
        if($order->paid_at || $order->closed){
            throw new InvalidRequestException('订单状态不正常');
        }
        ...
        return app('alipay')->web([
            ...
            'subject'=>'支付LaravelShop的订单:'.$order->no,// 订单标题
        ]);
    }
    ...
    public function payByInstallment(Order $order,Request $request){
        ...
        // 订单已支付或者已关闭
        if($order->paid_at||$order->closed){
            throw new InvalidArgumentException('订房状态不正确');
        }
        ...
        $installment = new Installment([
            'total_amount'=>$order->total_amount, // 总本金即为商品订单总金额
            'count'=>$count,// 分期期数
            'fee_rate'=>config('app.installment_fee_rate')[$count],// 从配置文件中读取相应期数的费率
            'fine_rate'=>config('app.installment_fine_rate'), // 从配置文件中读取当期逾期费率
        ]);
